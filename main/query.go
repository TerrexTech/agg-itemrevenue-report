...
func Query(itemSoldColl *mongo.Collection, reportColl *mongo.Collection, event *model.Event) *model.KafkaResponse {
	...
	if err != nil {
		err = errors.Wrap(err, "Query: Error while unmarshalling Event-data - ItemRevenueReport")
		logger.E(tlog.Entry{
	...
	if &filter == nil {
		...
		err = errors.Wrap(err, "Query left blank - ItemRevenueReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, filter)
	...
	if err != nil {
		err = errors.Wrap(err, "Error getting results from ItemRevenueReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, filter)
	...
	if len(avgSoldReport) < 1 {
		err = errors.New("Error: No result found from agg_itemrevenue collection - Function = ItemRevenueReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, reportAgg)
	...
	reportAgg = report.RevenueSoldWeight(avgSoldReport)
